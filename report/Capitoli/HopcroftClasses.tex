...
\begin{enumerate}
    \item \textbf{Height Computation:} The height of each state is determined, \draft{\sout{where the height of a state is the length of the longest path from that state to a final state.}} \alessio{È la definzione 5, non serve riscriverlo. Al massimo fai una ref.}
    ...
    \end{itemize}
    \alessio{Andrei più nello specifico, dopo riporti la label $(F/NF, l_1, nl_1, l_2, nl_2, \dots, l_k, nl_k)$, se sono la stessa cosa, conviene spiegarlo subito qua.}
...
In detail\draft{\sout{s}}, the algorithm minimizes a\draft{\sout{n acyclic deterministic finite automaton} DAWG} by leveraging the concept of state height. The algorithm partitions the states based on their height. It then processes these \draft{\sout{partitions} groups} in increasing order of height, starting from height 0.
...
This process uses a specialized lexicographic sorting technique (related to bucket sort) optimized for this task, which helps achieve linear time complexity relative to the size of the automaton (number of states and transitions). The algorithm leverages a technique similar to the one presented in \cite{aho1974design} for testing tree isomorphism. Specifically, Revuz adopt\draft{s} a renumbering scheme during the lexicographic sorting phase to optimize both time and space complexity. 

\subsection{Renumbering Scheme \draft{\sout{Explained}}}
\alessio{Maybe a subsection separates too much, try using a subsubsection/paragraph (without numbering).}
...
The renumbering scheme overcomes this issue by assigning temporary, small integer names to the target states ($nl_i$) during the sorting process for each height level $\Pi_i$. The key idea is that when sorting states at height $i$, we only need to distinguish between the equivalence classes of the target states \draft{$nl_i$} \alessio{È corretto il pedice $i$ o dovrebbe essere $j$?} from lower levels ($j < i$), as those levels have already been processed and minimized. The renumbering scheme ensures that the bucket array size is bounded by the maximum \draft{\sout{of} between} $|\draft{\sout{C} \Sigma}|$ (the alphabet size) and $\draft{|E_i|}$ (the total number of edges from states at height i). The renumbering function adopted is \draft{\sout{the following:} presented in Algorithm \cref{alg:renumbering}} \alessio{Non usare mai ``in the following'' o simili, perché a Latex piace muovere le cose e potrebbe essere poco chiaro. Usa sempre ref con i numeri.}
...
\draft{The paper presents} \alessio{Cita} the core minimization logic and a more detailed sorting (or distinguishing) algorithm separately. \draft{\sout{Here is} Algorithm \cref{alg:minimization-dawg} shows} a combined representation based on the `Final algorithm' section of the paper.

\begin{algorithm}
    \caption{Minimization Algorithm for \draft{\sout{Acyclic DFAs} DAWGs}}  
...
\draft{\sout{where $F/NF$ indicates final/non-final, $l_k$ is the $k$-th transition symbol, and $nl_k$ is the (renumbered) target state name.}} \alessio{Spiegato prima} The sorting happens component by component.
...
\subsection{Example} \alessio{Usa un example environment.}
...
\alessio{Ricorda di specificare che questo è un caso specifico e che potrebbe anche non essere un albero, ma a noi va bene così perché ci concentriamo su quelli.}
...
Now by applying the algorithm, we obtain minimized DAWG represented in figure \cref{fig:example_minimized_dawg}. \alessio{Mostra come! Fai vedere un po' di passaggi dell'algoritmo per mostrare bene come funziona.}
Each node of the original DAWG is represented by a node in the minimized DAWG (equivalence classes). The edges represent transitions between these nodes. The root node $A$ is the initial state of the minimized DAWG, while the node $D$ is the final state.
\begin{figure}[H]
    \centering
