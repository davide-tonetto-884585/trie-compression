...
public:
     ...
     */
    template <typename T>
    ChainsDivisionSolver(const TreeDAWG<T> &treeDAWG, const std::vector<uint64_t> &node_order, uint16_t p, bool optimized, bool verbose = false)
        : m_num_nodes(treeDAWG.get_num_nodes()), m_pm(2 * (treeDAWG.get_num_nodes() + p), 2 * (treeDAWG.get_num_nodes() * p) + (treeDAWG.get_num_nodes() * (treeDAWG.get_num_nodes() - 1)) / 2), m_p(p), m_chains(p), m_node_order(node_order)
    {
        if (verbose)
        {
        ...
        uint64_t max_eq_classes = 0;
        for (uint64_t i = 0; i < t; ++i)
        {
            if (max_eq_classes < treeDAWG[i].get_equivalence_class())
                max_eq_classes = treeDAWG[i].get_equivalence_class();
        }
        ...
        if (!optimized)
        {
            ...
            {
                uint64_t u_i = i + p;
                uint64_t u_i_equiv_class = treeDAWG[node_order[i]].get_equivalence_class();

                for (uint64_t index_p = 0; index_p < p; ++index_p)
                {
                    m_pm.AddEdge(index_p, t + p + i, 1);
                    m_pm.AddEdge(u_i, 2 * t + p + index_p, 0);
                }

                for (uint64_t j = i + 1; j < t; j++)
                {
                    uint64_t u_j = j + t + p;
                    uint64_t u_j_equiv_class = treeDAWG[node_order[j]].get_equivalence_class();
                    if (u_i_equiv_class == u_j_equiv_class)
                        m_pm.AddEdge(u_i, u_j, 0);
                    else
                        m_pm.AddEdge(u_i, u_j, 1);
                }
            }
        }
        else
        {
            ...
            {
                uint64_t p_count = 0;
                std::vector<uint64_t> p_classes{};
                for (uint64_t j = i + 1; j < t; j++)
                {
                    uint64_t u_j = j + t + p;
                    uint64_t u_j_equiv_class = treeDAWG[node_order[j]].get_equivalence_class();
                    if (p_count < p && std::find(p_classes.begin(), p_classes.end(), u_j_equiv_class) == p_classes.end())
                    {
                        m_pm.AddEdge(i, u_j, 1);
                        p_count++;
                        p_classes.push_back(u_j_equiv_class);
                    }
            ...
            for (uint64_t i = 0; i < t; i++)
            {
                ...
                for (uint64_t j = i + 1; j < t; j++)
                {
                    uint64_t u_j = j + t + p;
                    uint64_t u_j_equiv_class = treeDAWG[node_order[j]].get_equivalence_class();
                    if (u_i_equiv_class == u_j_equiv_class && !found)
                    {
                        m_pm.AddEdge(u_i, u_j, 0);
                        found = true;
                    }
                    else if (u_i_equiv_class != u_j_equiv_class && p_count < p && std::find(p_classes.begin(), p_classes.end(), u_j_equiv_class) == p_classes.end())
                    {
                        m_pm.AddEdge(u_i, u_j, 1);
                        p_count++;
                        p_classes.push_back(u_j_equiv_class);
            ...
            for (uint64_t i = 0; i < p; ++i)
            {
                for (uint64_t j = 0; j < p && j < class_change_pos.size(); ++j)
                {
                    uint64_t source_node = i;                                 // s_i in V1
                    uint64_t target_node = t + p + class_change_pos[j].first; // v_j in V2
            ...
            for (uint64_t i = 0; i < t; ++i)
            {
                ...
                while (cur_cp_index < class_change_pos.size() && i < t - 1)
                {
            ...
            if (num_0_edges != t - (max_eq_classes + 1))
                throw std::runtime_error("0-Edges num error: " + std::to_string(num_0_edges) + " != " + std::to_string(t - max_eq_classes + 1));

            if (num_dest != (max_eq_classes + 1) * p)
                throw std::runtime_error("Destination edges num error: " + std::to_string(num_dest) + " != " + std::to_string((max_eq_classes + 1) * p));
            */
        }
    }
